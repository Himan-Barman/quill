...
export default function DailyRevisionPage() {
  ...
  const initializeQuiz = () => {
    ...
    const generatedQuestions: QuizQuestion[] = todayWords.map((word) => {
      const correctAnswer = word.simple_meaning || word.meaning;
      const optionSet = new Set<string>([correctAnswer]);

      let attempts = 0;
      while (optionSet.size < 4 && attempts < 100) {
        const randWord = words[Math.floor(Math.random() * words.length)];
        const meaning = randWord.simple_meaning || randWord.meaning;
        if (meaning) optionSet.add(meaning);
        attempts++;
      }

      while (optionSet.size < 4) {
        optionSet.add(`Definition option ${optionSet.size + 1}`);
      }
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-background text-center">
        ...
        <h2 className="text-2xl font-bold text-foreground mb-2">No Words Added Today</h2>
        <p className="text-sm text-foreground-variant max-w-sm mb-6">
          Add some new vocabulary words today to start your daily revision challenge!
        </p>
        <Link
          href="/words/add"
        ...
        <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 flex flex-col justify-between">
          ...
          <div className="w-full pt-6 pb-4 flex items-center justify-between gap-4">
            ...
            <button
              ...
              onClick={handleNext}
              className="px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold shadow-lg shadow-blue-600/20 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none transition-all flex items-center gap-2 cursor-pointer"
            >
